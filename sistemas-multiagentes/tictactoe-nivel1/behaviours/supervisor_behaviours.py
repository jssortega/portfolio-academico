...
# ── Umbrales del revisor de inactividad ─────────────────────
# RevisarInactividadBehaviour recorre periódicamente los ocupantes
# de cada sala y emite advertencias cuando se detectan situaciones
# anómalas en la duración de su presencia. Los umbrales están
# pensados para una partida de tres en raya simulada (segundos,
# no minutos): si un alumno tarda mucho más de lo normal en jugar
# o el tablero deja de avanzar, el profesor lo verá en la pestaña
# de incidencias antes de que finalice la prueba.
#
# Los tres umbrales son configurables desde 'config_parametros'
# del agente, lo que permite al profesor ajustarlos al ritmo
# observado el día del examen sin tocar el código.

# Periodicidad del propio revisor.
INTERVALO_REVISION_INACTIVIDAD_S = 15

# Tiempo que un jugador puede llevar en la sala sin aparecer en
# ningún informe antes de emitir la advertencia. 90 s deja margen
# para que una partida estándar termine y el supervisor procese
# el game-report correspondiente.
UMBRAL_JUGADOR_SIN_PARTIDA_S = 90

# Tiempo que un tablero puede permanecer sin cambiar de presencia
# antes de avisar. Los tableros bien implementados van anunciando
# 'waiting' → 'playing' → 'finished'; un valor estancado durante
# este umbral suele indicar un fallo en su FSM.
UMBRAL_TABLERO_ESTADO_ESTANCADO_S = 90

# Tiempo que un tablero puede estar en 'playing' sin llegar nunca
# a 'finished'. Es más generoso que el anterior porque incluye la
# duración real de la partida. Si se supera, el tablero del alumno
# probablemente esté colgado o no haya implementado correctamente
# la transición final.
UMBRAL_TABLERO_PLAYING_SIN_FIN_S = 180

# Subtipos de advertencia que el revisor anota en el campo 'de'
# del evento, además del nick del afectado, para que el profesor
# pueda distinguirlos de un vistazo en la pestaña de incidencias.
SUBTIPO_JUGADOR_SIN_PARTIDA = "jugador_sin_partida"
SUBTIPO_TABLERO_SIN_CAMBIOS = "tablero_sin_cambios_de_estado"
SUBTIPO_TABLERO_PLAYING_SIN_FIN = "tablero_playing_sin_finalizar"

# Estados de presencia MUC reconocidos del tablero. Las constantes
# se usan en el revisor para distinguir los casos 'no avanzó' y
# 'no terminó'.
ESTADO_TABLERO_PLAYING = "playing"
ESTADO_TABLERO_FINISHED = "finished"
...
def _identidades_jugadores_en_informes(
    informes_por_sala: dict,
) -> set[str]:
    """Extrae todos los nicks y JIDs (bare y locales) de jugadores
    que aparecen como ``X`` u ``O`` en algún informe de la sala.

    Acepta los tres formatos en los que el supervisor puede haber
    recibido el JID del jugador en el game-report:

    * JID con recurso ``jugador_ana@localhost/abcd``,
    * JID bare ``jugador_ana@localhost``,
    * solo nick ``jugador_ana`` (cuando el tablero del alumno solo
      registró el apodo MUC).

    Args:
        informes_por_sala: Diccionario ``{jid_tablero: [cuerpo, …]}``
            del agente, indexado por JID del tablero.

    Returns:
        Conjunto con todas las identidades equivalentes. Se usa
        para decidir si un ocupante presente en la sala "ha
        jugado", aceptando cualquier representación del mismo
        agente.
    """
    identidades: set[str] = set()
    for lista_cuerpos in informes_por_sala.values():
        for cuerpo in lista_cuerpos:
            jugadores = cuerpo.get("players", {}) or {}
            for ficha in ("X", "O"):
                valor = jugadores.get(ficha, "") or ""
                if not valor:
                    continue
                ...
                if "/" in valor:
                    ...
                    identidades.add(valor)
                if "@" in valor:
                    identidades.add(valor.split("@", 1)[0])
    return identidades


def _ha_jugado(ocupante: dict, identidades_jugadoras: set[str]) -> bool:
    """Indica si un ocupante aparece en algún informe procesado.

    Compara tanto el ``nick`` MUC como el ``jid`` bare del
    ocupante con las identidades extraídas de los informes. Basta
    con que una de las dos formas coincida.
    """
    nick = ocupante.get("nick", "")
    jid = ocupante.get("jid", "")
    coincide_nick = bool(nick) and nick in identidades_jugadoras
    coincide_jid = bool(jid) and jid in identidades_jugadoras
    return coincide_nick or coincide_jid


class RevisarInactividadBehaviour(PeriodicBehaviour):
    """Detecta y registra como advertencias situaciones anómalas de
    presencia que el resto del supervisor no captura por sí solo.

    Tres patrones se vigilan en cada pasada:

    1. **Jugador sin partida**: un ocupante con prefijo
       ``jugador_`` lleva más de
       :data:`UMBRAL_JUGADOR_SIN_PARTIDA_S` segundos en la sala y
       no aparece como ``X`` ni como ``O`` en ningún informe
       procesado. Se identifica con el subtipo
       :data:`SUBTIPO_JUGADOR_SIN_PARTIDA`.
    2. **Tablero sin cambios de estado**: un ocupante con prefijo
       ``tablero_`` no ha cambiado su presencia desde hace más de
       :data:`UMBRAL_TABLERO_ESTADO_ESTANCADO_S` segundos —y el
       estado actual NO es ``finished``, porque un tablero ya
       finalizado no tiene que cambiar nada—. Subtipo
       :data:`SUBTIPO_TABLERO_SIN_CAMBIOS`.
    3. **Tablero en ``playing`` que no finaliza**: el tablero entró
       en estado ``playing`` hace más de
       :data:`UMBRAL_TABLERO_PLAYING_SIN_FIN_S` segundos y sigue
       sin pasar a ``finished``. Subtipo
       :data:`SUBTIPO_TABLERO_PLAYING_SIN_FIN`.

    Las tres advertencias se emiten **una sola vez por
    (sala, nick, subtipo)** mediante el conjunto
    ``alertas_inactividad_emitidas`` del agente. Esto evita
    inundar la pestaña de incidencias del panel con la misma
    advertencia en cada pasada del temporizador.

    El behaviour es defensivo: si los marcadores temporales del
    ocupante (``joined_at`` o ``estado_cambiado_at``) no están
    disponibles —por ejemplo en tests que construyen el agente sin
    pasar por presencia—, la comprobación se omite para ese
    ocupante sin propagar ningún error.
    """

    async def run(self) -> None:
        """Realiza una pasada de revisión en todas las salas."""
        ahora = datetime.now()
        umbral_jugador = float(getattr(
            self.agent, "umbral_jugador_sin_partida_s",
            UMBRAL_JUGADOR_SIN_PARTIDA_S,
        ))
        umbral_tablero_estado = float(getattr(
            self.agent, "umbral_tablero_estado_estancado_s",
            UMBRAL_TABLERO_ESTADO_ESTANCADO_S,
        ))
        umbral_tablero_playing = float(getattr(
            self.agent, "umbral_tablero_playing_sin_fin_s",
        ...
        for sala in self.agent.salas_muc:
            self._revisar_sala(
                sala["id"], ahora,
                umbral_jugador, umbral_tablero_estado,
                umbral_tablero_playing,
            )

    def _revisar_sala(
        self, sala_id: str, ahora: datetime,
        umbral_jugador: float,
        umbral_tablero_estado: float,
        umbral_tablero_playing: float,
    ) -> None:
        """Recorre los ocupantes de una sala y emite las
        advertencias procedentes."""
        ocupantes = self.agent.ocupantes_por_sala.get(sala_id, [])
        if not ocupantes:
            return

        identidades_jugadoras = _identidades_jugadores_en_informes(
            self.agent.informes_por_sala.get(sala_id, {}),
        )

        for ocupante in ocupantes:
            nick = ocupante.get("nick", "")
            if not nick or nick == getattr(
                self.agent, "muc_apodo", "supervisor",
            ):
                # El propio supervisor nunca se inspecciona.
                continue

            if nick.startswith("jugador_"):
                self._revisar_jugador(
                    sala_id, ocupante, ahora,
                    umbral_jugador, identidades_jugadoras,
                )
            elif nick.startswith("tablero_"):
                self._revisar_tablero(
                    sala_id, ocupante, ahora,
                    umbral_tablero_estado,
                    umbral_tablero_playing,
                )

    def _revisar_jugador(
        self, sala_id: str, ocupante: dict, ahora: datetime,
        umbral_s: float, identidades_jugadoras: set[str],
    ) -> None:
        """Aplica el criterio de inactividad a un jugador."""
        joined_at = ocupante.get("joined_at")
        if joined_at is None:
            return
        if _ha_jugado(ocupante, identidades_jugadoras):
            return
        if (ahora - joined_at).total_seconds() < umbral_s:
            return
        nick = ocupante["nick"]
        if self._ya_alertado(
            sala_id, nick, SUBTIPO_JUGADOR_SIN_PARTIDA,
        ):
            return
        self._marcar_alertado(
            sala_id, nick, SUBTIPO_JUGADOR_SIN_PARTIDA,
        )
        detalle = (
            f"Jugador '{nick}' lleva en la sala "
            f"{int((ahora - joined_at).total_seconds())} s sin "
            "aparecer en ningún informe de partida"
        )
        self.agent.registrar_evento_log(
            LOG_ADVERTENCIA, SUBTIPO_JUGADOR_SIN_PARTIDA,
            detalle, sala_id,
        )

    def _revisar_tablero(
        self, sala_id: str, ocupante: dict, ahora: datetime,
        umbral_estado_s: float, umbral_playing_s: float,
    ) -> None:
        """Aplica los criterios de inactividad a un tablero."""
        nick = ocupante["nick"]
        estado = ocupante.get("estado", "")

        # Caso 3: 'playing' que no llega a 'finished'.
        if estado == ESTADO_TABLERO_PLAYING:
            cambiado_at = ocupante.get("estado_cambiado_at")
            if cambiado_at is None:
                return
            transcurridos = (ahora - cambiado_at).total_seconds()
            if (
                transcurridos >= umbral_playing_s
                and not self._ya_alertado(
                    sala_id, nick,
                    SUBTIPO_TABLERO_PLAYING_SIN_FIN,
                )
            ):
                self._marcar_alertado(
                    sala_id, nick,
                ...
                detalle = (
                    f"Tablero '{nick}' lleva {int(transcurridos)} s "
                    f"en estado '{ESTADO_TABLERO_PLAYING}' sin "
                    f"alcanzar '{ESTADO_TABLERO_FINISHED}'"
                )
                self.agent.registrar_evento_log(
                    LOG_ADVERTENCIA,
                    SUBTIPO_TABLERO_PLAYING_SIN_FIN,
        ...
        # Caso 2: presencia estancada (no aplica una vez finalizado:
        # un tablero ya 'finished' no tiene por qué cambiar de
        # estado, así que evitamos un aviso superfluo).
        if estado == ESTADO_TABLERO_FINISHED:
            return
        cambiado_at = ocupante.get("estado_cambiado_at")
        if cambiado_at is None:
            return
        transcurridos = (ahora - cambiado_at).total_seconds()
        if transcurridos < umbral_estado_s:
            return
        if self._ya_alertado(
            sala_id, nick, SUBTIPO_TABLERO_SIN_CAMBIOS,
        ):
            return
        self._marcar_alertado(
            sala_id, nick, SUBTIPO_TABLERO_SIN_CAMBIOS,
        ...
        detalle = (
            f"Tablero '{nick}' lleva {int(transcurridos)} s en el "
            f"estado '{estado or '(sin estado)'}' sin progresar"
        )
        self.agent.registrar_evento_log(
            LOG_ADVERTENCIA, SUBTIPO_TABLERO_SIN_CAMBIOS,
            detalle, sala_id,
        )

    # ── Gestión del conjunto de alertas ya emitidas ──────────

    def _ya_alertado(
        self, sala_id: str, nick: str, subtipo: str,
    ) -> bool:
        emitidas = getattr(
            self.agent, "alertas_inactividad_emitidas", None,
        )
        if emitidas is None:
            return False
        return (sala_id, nick, subtipo) in emitidas

    def _marcar_alertado(
        self, sala_id: str, nick: str, subtipo: str,
    ) -> None:
        emitidas = getattr(
            self.agent, "alertas_inactividad_emitidas", None,
        ...
        if emitidas is None:
            self.agent.alertas_inactividad_emitidas = {
                (sala_id, nick, subtipo),
            }
        else:
            emitidas.add((sala_id, nick, subtipo))
...
class EstadoEsperarRespuesta(State):
    ...
    async def run(self) -> None:
        ...
        # Reset defensivo: SPADE reusa instancias de State sin
        # limpiar ``next_state`` entre vueltas (ver comentario en
        # EstadoRegistrarTimeout.run).
...
class EstadoEsperarInforme(State):
    ...
    async def run(self) -> None:
        ...
        # Reset defensivo: ver comentario en EstadoRegistrarTimeout.run.
        self.set_next_state(None)
...
class EstadoProcesarInforme(State):
    ...
    async def run(self) -> None:
        ...
        # Estado terminal: garantizamos que SPADE no herede una
        # transición previa (ver EstadoRegistrarTimeout.run).
        self.set_next_state(None)
...
class EstadoProcesarRechazo(State):
    ...
    async def run(self) -> None:
        ...
        # Estado terminal: ver comentario en EstadoRegistrarTimeout.run.
        self.set_next_state(None)
...
class EstadoRegistrarTimeout(State):
    ...
    async def run(self) -> None:
        ...
        # SPADE reusa la misma instancia de State entre iteraciones del
        # FSM y NO resetea ``next_state`` antes de cada ``run()``. Si en
        # una vuelta anterior se llamó a ``set_next_state(ST_REINTENTAR)``
        # y ahora entramos en la rama de "agotados" sin llamar a
        # ``set_next_state(...)``, SPADE leería el valor heredado y
        # transitaria de nuevo a REINTENTAR, ignorando el límite de
        # reintentos. Reseteamos explícitamente al principio.
        self.set_next_state(None)
