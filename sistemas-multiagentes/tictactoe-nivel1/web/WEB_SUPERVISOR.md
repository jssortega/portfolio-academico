...
Los tipos de evento y su codificación visual:
...
| `solicitud` | ▸     | azul     | Solicitud de informe enviada al        |
|             |       |          | tablero (REQUEST game-report).         |
| `informe`   | ★     | amarillo | Informe de partida recibido.           |
| `abortada`  | ⚠     | naranja  | Partida abortada por incidencia.       |
| `timeout`   | ⏱     | naranja  | Tablero sin respuesta tras tiempo de espera. |

---

## Interacciones del usuario

El panel ofrece 6 interacciones principales, todas implementadas
...
- **Acción:** Clic en un tab (Informes, Agentes, Clasificación, Registro).
...
- **Concepto pedagógico:** Los cuatro paneles representan las cuatro
  dimensiones de la supervisión: **resultados** (informes),
  **presencia** (agentes), **rendimiento** (clasificación) y
...
## Ciclo de consulta periódica y flujo de datos

La comunicación entre la interfaz y el servidor se realiza mediante
**consulta periódica HTTP** (peticiones periódicas), no mediante
WebSocket ni Server-Sent Events. Esta decisión prioriza la simplicidad
y la transparencia didáctica.
...
|------------------------|-----------|-----------------------------------|----------------------------------|
| Estado en vivo         | 5 s       | `GET /supervisor/api/state`       | Datos de todas las salas.         |
