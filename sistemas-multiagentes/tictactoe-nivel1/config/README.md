...
**Perfil LLM** (la estrategia nivel 4 es opcional). El campo
`llm.perfil_activo` admite tres valores:

| Perfil | Proveedor | Servicio | Modelo |
|--------|-----------|----------|--------|
| `ninguno` | — | No se usa ningún LLM (**valor por defecto**) | — |
| `gemini` | Google AI Studio | API en la nube (clave personal) | `gemini-2.5-flash` |
| `servidor` | Ollama | `http://sinbad2ia.ujaen.es:8050` | `ollama_chat/llama3:8b` |

Con `ninguno` el sistema arranca sin pedir ninguna API key, por lo
que es la opción correcta para los alumnos que solo emplean los
niveles de estrategia 1 a 3.

**Parámetros del sistema:** intervalos de búsqueda MUC, máximo de
partidas simultáneas, tiempos de espera (*timeout*) de inscripción y
turno, puerto web base y nivel de registro (*logging*).

Para cambiar de entorno, edita el campo `perfil_activo` en la sección
correspondiente. Los dos selectores son **independientes**: el de
XMPP (`xmpp.perfil_activo`) y el de LLM (`llm.perfil_activo`) se
ajustan por separado, cambiar uno no modifica el otro. El resto del
sistema lee automáticamente los datos del perfil seleccionado.
...
Tanto `main.py` como `conftest.py` importan estas funciones, de modo
que agentes y pruebas leen siempre los mismos parámetros de la misma
fuente.

## Recordatorio

- Nunca escribir directamente en el código URL, puertos, nombres de
  servidor ni contraseñas. Todo debe proceder de `config.yaml`.
- El cambio entre entorno local y servidor se realiza editando los
  campos `perfil_activo`, sin modificar nada más.
