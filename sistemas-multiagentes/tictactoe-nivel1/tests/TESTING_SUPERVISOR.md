...
├── test_almacen_supervisor.py     ← 25 tests — persistencia SQLite
├── test_supervisor_behaviours.py  ← 27 tests — funciones auxiliares y FSM
├── test_agente_supervisor.py      ← 21 tests — métodos del agente y presencia
├── test_supervisor_handlers.py    ← 23 tests — conversión de datos y rutas HTTP
├── test_creacion_salas.py         ← 13 tests — creación de salas MUC por modo
└── TESTING_SUPERVISOR.md          ← Este documento
```

**Total: 169 tests** ejecutables con `pytest tests/ -v`.
...
| `test_supervisor_behaviours.py` | 32 | `_determinar_rol`, `_construir_detalle_informe`, los 6 estados de `SolicitarInformeFSM` y validación de ontología |
| `test_almacen_supervisor.py` | 25 | `AlmacenSupervisor`: inicialización, ejecuciones, informes, eventos y aislamiento |
| `test_supervisor_handlers.py` | 25 | `_mapear_resultado`, `_nombre_legible_sala`, `_convertir_informes` (JID MUC vs real) y rutas HTTP |
| `test_agente_supervisor.py` | 34 | `_identificar_sala`, `obtener_sala_de_tablero`, `registrar_evento_log` y `_on_presencia_muc` (entradas, salidas, cambios de estado, detección de tableros finalizados) |
| `test_creacion_salas.py` | 20 | Carga de `salas_laboratorio.yaml`, `sala_torneo.yaml` y `torneos.yaml`; creación y visibilidad de salas MUC |
| `test_integracion_supervisor.py` | 14 | Tests de integración con agentes SPADE reales (requiere `spade run`). Ver [TESTING_INTEGRACION_SUPERVISOR.md](TESTING_INTEGRACION_SUPERVISOR.md) |
    ...
    def __init__(self):
        ...
        self.log_eventos = [
            {"ts": "09:35:12", "tipo": "informe",
             "de": "tablero_mesa2",
             "detalle": "Empate · luis vs ana · 9 turnos"},
            {"ts": "09:28:30", "tipo": "informe",
             "de": "tablero_mesa1",
             "detalle": "Victoria X (ana) vs luis · 7 turnos"},
...
LOG_EJEMPLO = [
    {"ts": "09:28:30", "tipo": "informe", "de": "tablero_mesa1",
     "detalle": "Victoria X (ana) vs luis · 7 turnos"},
    {"ts": "09:25:00", "tipo": "presencia", "de": "jugador_luis",
     "detalle": "Se une a la sala MUC"},
    {"ts": "09:24:50", "tipo": "presencia", "de": "jugador_ana",
     "detalle": "Se une a la sala MUC"},
]
```
