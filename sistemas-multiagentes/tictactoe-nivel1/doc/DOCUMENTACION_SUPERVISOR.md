    ...
    "salas": [
        {
            ...
            "log": [
                { "ts": "16:05:23", "tipo": "informe", "de": "tablero_mesa1", "detalle": "Victoria X (alice) vs bob · 7 turnos" }
            ]
        },
        {
            "id": "torneo",
...
- **Tests unitarios:** 186 tests en 5 ficheros que se ejecutan sin
  SPADE ni servidor XMPP, utilizando objetos simulados.
- **Tests de integración:** 14 tests que arrancan agentes SPADE reales
  contra un servidor XMPP local (`spade run`) y verifican escenarios
  completos de laboratorio con agentes simulados.
...
|---------|------:|----------------------|
| `tests/test_almacen_supervisor.py` | 25 | Capa de persistencia SQLite (`AlmacenSupervisor`) |
| `tests/test_supervisor_behaviours.py` | 32 | Funciones auxiliares, los 6 estados del FSM y validación de ontología |
| `tests/test_agente_supervisor.py` | 34 | Métodos del agente y manejador de presencia MUC (`_on_presencia_muc`) |
| `tests/test_supervisor_handlers.py` | 25 | Funciones de conversión (incl. JID MUC vs real) y las 4 rutas HTTP |
| `tests/test_creacion_salas.py` | 20 | Creación de salas MUC y visibilidad en el supervisor |
| `tests/test_integracion_supervisor.py` | 14 | Tests de integración con agentes SPADE reales (requiere `spade run`) |
...
| `TestFinalizarEjecucion` | Establece la fecha de fin, la ejecución activa tiene fin nulo, y finalizar sin ejecución no lanza excepciones |
