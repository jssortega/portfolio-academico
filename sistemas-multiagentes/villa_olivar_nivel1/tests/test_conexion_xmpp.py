"""
Agente de prueba para verificar la conexión XMPP.
...
# Se usa un nombre genérico para evitar colisiones. Cada grupo puede
# sustituir "prueba_conexion" por un identificador propio si lo desea.
USUARIO_PRUEBA = "prueba_conexion"
CONTRASENA_PRUEBA = "prueba_conexion_pass"
...
async def probar_conexion_spade(
    ...
    logger.info("  Intentando conectar agente: %s@%s (puerto %d)",
                USUARIO_PRUEBA, servidor, puerto)

    conexion_exitosa = False
    agente = crear_agente(Agent, USUARIO_PRUEBA, CONTRASENA_PRUEBA,
                          config_xmpp)
...
async def probar_perfil(nombre_perfil: str,
    ...
    logger.info("=" * 60)
    logger.info("Probando perfil: %s (%s:%d)", nombre_perfil, servidor, puerto)
    logger.info("=" * 60)
    ...
    if not puerto_abierto:
        ...
        logger.error("    - El nombre de host '%s' no se resuelve", servidor)
        resultado = False
    else:
...
async def ejecutar_pruebas(perfiles_a_probar: list[str]) -> None:
    ...
    Args:
        perfiles_a_probar: Lista con los nombres de los perfiles a verificar.
    """
    config = cargar_configuracion()
    perfiles_xmpp = config["perfiles_xmpp"]
