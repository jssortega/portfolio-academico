...
public:
    ...
    /**@brief Añade un elemento en cualquier posición del vector desplazando el resto a la _dch
    ...
    void ordenarRev();
    /**@brief busca un _dato en el vector
     * @param dato: el _dato que quieres buscar
     * @return la posicion en la que se encuentra el _dato
     */
...
template<typename T>
VDinamico<T>::VDinamico(unsigned int taml): _taml(0),_tamf(pow(2,ceil((log2(taml))))) {
    v = new T[_tamf];
}
